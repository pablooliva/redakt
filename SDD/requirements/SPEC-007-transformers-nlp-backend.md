...
2. **Held-out positive bar (new):** at least one positive German `LOCATION` fixture and at least one positive German `DATE_TIME` fixture (added per REQ-009b — see below) appear in `tests/eval/fixtures/de.yaml` with their expected entities listed; these fixtures MUST produce a `found` set that contains the expected `LOCATION` / `DATE_TIME` entity (verified by the harness branch that asserts presence — i.e., a *true-positive* assertion, distinct from `expect_clean` and from `issubset`-only). This catches threshold values that drop legitimate hits.
...
#### REQ-009b: Held-out positive fixtures (DE LOCATION, DE DATE_TIME) and long-document anchor
Add three new fixtures to `tests/eval/fixtures/de.yaml` whose expected-entity sets are non-empty (true-positive assertions, distinct from `expect_clean`):

1. **DE LOCATION positive:** a sentence-context fixture such as `Sie wohnt in Berlin und arbeitet in München.`, with `expected_entities: [LOCATION, LOCATION]`. The harness MUST assert that `LOCATION` is present in `found` (not merely `issubset`-passed against an empty expected set). This is the held-out positive anchor for REQ-006 bar 2.
2. **DE DATE_TIME positive:** a sentence-context fixture such as `Der Termin ist morgen um 14 Uhr.`, with `expected_entities: [DATE_TIME]`. Same true-positive assertion shape.
3. **Long-document anchor (>500 transformer tokens):** a German-language fixture whose tokenized length under `xlm-roberta-large-finetuned-conll03-german`'s tokenizer exceeds 500 tokens (concrete content: a synthetic ~200-word German prose paragraph repeated until token count threshold, OR a single long German narrative — the implementer chooses; tokenized length MUST be recorded in the fixture or report). This fixture is referenced by EDGE-006 ("at least one fixture with German text >500 tokens") and PERF-001's long-document anchor, eliminating the iter 2 backfill ambiguity.

**Acceptance:**
- `tests/eval/fixtures/de.yaml` contains three new entries matching (1)–(3); `uv run pytest tests/eval/` is green (the eval suite total becomes 59 = 41 existing + 15 broader-class clean + 3 held-out positive/long-doc).
- The harness asserts the held-out positive entities are present in `found` for fixtures (1) and (2). If `tests/eval/test_calibration.py` does not already support a "must contain" branch (verified during step 7 implementation), the harness gains a small extension in the same PR; failure mode is a meaningful pytest failure (`AssertionError` listing the missing entity), not a silent pass.
- The long-document fixture (3) is exercised by `tools/calibration_report.py --raw --out` and the resulting transcript captured in `reports/`.
...
**Risk:** Medium. Wrong values cause silent under- or over-redaction in production. Mitigation: REQ-006 / REQ-007 acceptance now hinges on a four-bar stopping condition — (1) negative bar (existing fixtures + `expect_clean` fixtures stay PASS), (2) held-out positive bar (REQ-009b held-out DE LOCATION + DE DATE_TIME true-positive assertions), (3) score-distribution justification in the committed calibration report, (4) reproducibility within ±0.05. The bidirectional bar plus held-out positives plus written rationale catches wrong-direction tuning AND prevents a one-shot tune that doesn't replay (the original Medium framing's "circular acceptance" critique is closed by bars 2–4).
...
- **MODULE-005 / REQ-009 / REQ-009b / REQ-010:** `uv run pytest tests/eval/` returns 59/59 PASS (41 existing + 15 new `expect_clean: true` broader-class + 3 new held-out positives / long-document anchor per REQ-009b). API contract preservation (REQ-010): `tests/` (unit + integration, ignoring eval and e2e) green; `tests/e2e/` green; OpenAPI schema diff zero.
...
6. **MODULE-006:** Run `tools/calibration_report.py --raw --out`. Read the `graded scores` distribution. Re-tune `entity_score_thresholds` (Redakt) and `low_score_entity_names` / `low_confidence_score_multiplier` (analyzer-side, `de` row). Iterate until **all four bars** of REQ-006's stopping condition hold: (1) full fixture set PASS including `expect_clean` from step 7; (2) held-out positive DE LOCATION + DE DATE_TIME fixtures (REQ-009b) produce expected entities in `found`; (3) calibration report committed with score-distribution annotation per tuned threshold; (4) re-run reproducibility within ±0.05. If any bar fails, do not commit thresholds — iterate or change approach.
7. **MODULE-005:** Add the 15 new `expect_clean: true` fixtures to `tests/eval/fixtures/de.yaml` per REQ-009. Add the 3 new fixtures from REQ-009b (held-out DE LOCATION + DE DATE_TIME positives + long-document anchor). If a harness extension is needed for the "must contain" assertion (REQ-009b), land it in this PR. Run `uv run pytest tests/eval/` and verify 59/59 PASS.
